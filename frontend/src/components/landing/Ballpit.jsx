// Vendored from React Bits (https://reactbits.dev), Ballpit component —
// JS + CSS variant, upstream source is minified/obfuscated single-letter
// identifiers. Kept as-is (untyped, allowJs — see tsconfig.app.json) except
// for fixing literal backslash-escaped `!` characters introduced by copy/paste.
...
class x {
  ...
  dispose() {
    ...
    // No forceContextLoss(): React 18 StrictMode mounts this component,
    // cleans it up, then remounts it on the SAME canvas element within the
    // same tick. forceContextLoss() plus an immediate new WebGLRenderer on
    // that canvas leaves some GPU/driver stacks (confirmed on ANGLE/D3D11)
    // returning a context where getShaderPrecisionFormat is null, which
    // crashes WebGLCapabilities' precision check on the very first render.
    // renderer.dispose() alone already frees the GL resources.
...
function TouchStart(e) {
  if (e.touches.length > 0) {
    ...
    // Only steal the touch (preventDefault) when it actually lands on a
    // tracked canvas — the upstream version did this unconditionally on
    // every document-wide touchstart, which silently broke touch-scrolling
    // on the whole page for as long as any instance was mounted.
    let hitCanvas = false;
    for (const [elem, t] of b) {
...
class W {
  ...
  #R() {
    ...
    for (let i = 1; i < e.count; i++) {
      ...
      t[s + 1] = E(2 * e.maxY);
      t[s + 2] = E(2 * e.maxZ);
      // A small starting nudge so gravity:0 instances aren't dead still on
      // the very first frame, before `wander` (if any) has had time to act.
      v[s] = E(e.maxVelocity);
      v[s + 1] = E(e.maxVelocity);
      v[s + 2] = E(e.maxVelocity);
    }
  }
  setSizes() {
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      if (t.wander > 0) {
        B.x += E(t.wander) * e.delta;
        B.y += E(t.wander) * e.delta;
        B.z += E(t.wander) * e.delta;
      }
...
// vColor.rgb below (not bare vColor) because three@0.185.1's instanced-color
// path declares `varying vec4 vColor` — the upstream snippet's bare `vColor`
// assumed vec3 and failed fragment shader compilation with a dimension
// mismatch against this three version.
class Y extends c {
  constructor(e) {
    super(e);
    this.uniforms = {
...
const X = {
  ...
  friction: 0.9975,
  wallBounce: 0.95,
  // Continuous random nudge applied to every instance every frame (see
  // `update()`'s wander step) — without it, gravity:0 balls that start at
  // rest just sit there except when something else collides into them.
  // This keeps them drifting and bouncing off the walls indefinitely.
  wander: 0,
  maxVelocity: 0.15,
  maxX: 5,
  maxY: 5,
